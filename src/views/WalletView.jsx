                ...
                <SpotlightCard className="bg-gradient-to-br from-indigo-600 to-violet-900 rounded-[56px] p-8 sm:p-12 text-white shadow-[0_30px_80px_rgba(79,70,229,0.3)] group">
                    <div className="flex flex-col relative z-10 w-full h-full cursor-pointer" onClick={() => setIsFlipped(!isFlipped)}>
                        <div className={`transition-all duration-500 ${isFlipped ? 'opacity-0 absolute' : 'opacity-100'}`}>
                            ...
                            <h2 className="text-5xl sm:text-6xl lg:text-7xl font-black italic mb-12 leading-none tracking-tighter break-words"><ScrambleText text={formatNumber(userPoints)} /> <span className="text-sm not-italic opacity-60 block mt-4 uppercase tracking-widest font-bold">TrustPoints</span></h2>
                        </div>
                ...
                <div className="bg-[#0f172a]/60 rounded-[56px] p-12 border border-white/5 shadow-2xl backdrop-blur-xl">
                    ...
                    {/* AnalyticsGraph removed for MVP slimdown */}
                                        <div className="space-y-6 mt-8">
                                            {transactions.map(tx => (
                                                <div key={tx.id} className="flex flex-col justify-between min-h-[120px] p-8 bg-white/[0.02] rounded-[36px] border border-white/5 hover:bg-white/[0.05] transition-all group cursor-pointer">
                                                    ...
                                                    <div className="flex items-end self-end">
                                                        <p className={`text-3xl font-black italic tracking-tight ${tx.type === 'in' ? 'text-emerald-500' : 'text-slate-300'} whitespace-nowrap`}>{tx.type === 'in' ? '+' : '-'}{formatNumber(tx.points)}</p>
                                                        <span className="ml-2 text-[9px] font-black text-slate-600 uppercase tracking-[0.2em] leading-none">PTS</span>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
