  ...
  <div className="fixed top-24 right-6 z-[70] flex flex-col gap-3 pointer-events-none">
    {toasts.map(toast => (
      <div key={toast.id} className="pointer-events-auto bg-[#0f172a]/95 border border-white/10 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-4 animate-slide-in-right max-w-sm backdrop-blur-xl">
        <div className={`p-2 rounded-full ${toast.type === 'error' ? 'bg-rose-500/20 text-rose-400' : 'bg-emerald-500/20 text-emerald-400'}`}>
...
const ProfileModal = ({ isOpen, onClose, profile, actionLabel, onAction, addToast }) => {
  ...
  // Handler for Main Action (Initialize Contract)
  const handleAction = () => {
      if (onAction) {
          onAction(); // Trigger the navigation in parent
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl bg-[#0f172a] border border-white/10 rounded-[48px] shadow-2xl overflow-hidden animate-scale-up flex flex-col md:flex-row">
        ...
        <div className="md:w-1/3 bg-white/[0.02] p-8 border-b md:border-b-0 md:border-r border-white/5 flex flex-col items-center text-center relative overflow-hidden">
            {/* 修正: pointer-events-none を追加してクリック透過に */}
            ...
            <div className="relative z-10 w-32 h-32 rounded-full p-1 bg-gradient-to-tr from-indigo-500 to-emerald-500 mb-6 pointer-events-none">
                ...
                <div className="absolute bottom-1 right-1 w-8 h-8 bg-[#0f172a] rounded-full border border-white/10 flex items-center justify-center text-emerald-400">
            ...
            <div className="space-y-4 w-full relative z-10 pointer-events-none">
                <div className="flex items-center gap-3 text-slate-400 text-xs font-bold bg-white/5 p-3 rounded-xl">
                    <MapPin className="w-4 h-4 text-indigo-500" /> {profile.location || "Tokyo, Japan"}
                </div>
                <div className="flex items-center gap-3 text-slate-400 text-xs font-bold bg-white/5 p-3 rounded-xl">
                    <Calendar className="w-4 h-4 text-indigo-500" /> Member since {profile.joined || "2024"}
                </div>
                <div className="flex items-center gap-3 text-slate-400 text-xs font-bold bg-white/5 p-3 rounded-xl">
                    <Hexagon className="w-4 h-4 text-indigo-500" /> Level {profile.level || "42"} Architect
                </div>
            </div>

            {/* 修正: relative z-20 を追加して最前面に */}
            <button
                onClick={(e) => {
                    e.stopPropagation(); // バブリング防止
                    actionLabel ? handleAction() : handleExport();
                }}
                className={`mt-auto w-full py-4 flex items-center justify-center gap-2 text-xs font-black uppercase tracking-widest transition-colors relative z-20 ${actionLabel ? 'bg-indigo-600 text-white hover:bg-indigo-500 rounded-2xl mb-2 shadow-lg' : 'text-slate-500 hover:text-white'}`}
            >
                {actionLabel ? (
                    <>
        ...
        <div className="flex-1 p-8 md:p-12 space-y-10 relative">
             {/* 修正: コンテンツエリアにも念のため z-index 指定 */}
            <div className="flex justify-between items-center relative z-10">
                <h3 className="text-xl font-black text-white flex items-center gap-3">
                    <Activity className="w-5 h-5 text-emerald-500" /> Performance Metrics
                </h3>
                ...
                {['Reliability', 'Speed', 'Quality', 'Communication'].map((stat, i) => (
                    <div key={stat} className="p-4 bg-white/[0.02] rounded-2xl border border-white/5">
                        <div className="flex justify-between text-xs text-slate-400 font-bold mb-2">
                            <span>{stat}</span>
                            <span className="text-indigo-400">9{i+5}%</span>
                        </div>
                        <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                            <div className="h-full bg-gradient-to-r from-indigo-500 to-emerald-500" style={{ width: `9${i+5}%` }} />
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="relative z-10">
                ...
                <div className="flex gap-1 h-12">
                    {[...Array(30)].map((_, i) => (
                        <div
                            key={i}
                            className={`flex-1 rounded-sm ${Math.random() > 0.5 ? 'bg-indigo-500/80' : Math.random() > 0.5 ? 'bg-indigo-500/40' : 'bg-white/5'}`}
            ...
            <div className="p-6 bg-indigo-900/20 border border-indigo-500/20 rounded-3xl flex items-center gap-4 relative z-10">
                ...
                <div>
                    <h4 className="text-white font-bold">TrustFlow Guarantee</h4>
                    <p className="text-xs text-indigo-300">Identity verified via decentralized ledger.</p>
                </div>
                <div className="ml-auto text-2xl font-black text-white italic">100%</div>
            </div>
        </div>

        <button onClick={onClose} className="absolute top-6 right-6 p-2 bg-white/5 rounded-full text-slate-400 hover:text-white hover:bg-white/10 transition-colors z-50">
            <X className="w-5 h-5" />
        </button>
      </div>
    </div>
...
const App = () => {
  ...
  const toggleMode = () => {
    ...
    setTimeout(() => {
      setMode(prev => {
        const newMode = prev === 'earner' ? 'hirer' : 'earner';
        setView('marketplace');
        return newMode;
      });
  ...
  const handleAIArchitectSubmit = () => {
      ...
      setTimeout(() => {
          setAiSuggestions({
              summary: "Based on your request, I've architected a project scope.",
              dod: ["React Native Codebase", "Stripe Integration", "Biometric Auth Flow"],
              budget: "250,000 - 300,000 PTS",
              candidates: talents
          });
          setStatus('idle');
      }, 1500);
  ...
  const handleNextStep = () => {
    ...
    setTimeout(() => {
      if (step === 2) {
         if (mode === 'hirer') setUserPoints(p => p - selectedItem.totalPoints);
         else setUserPoints(p => p + selectedItem.totalPoints);
      }
      if (step === 3) {
  ...
  const commands = [
      ...
      { id: 'profile', label: 'View Trust Passport', icon: User, action: () => handleViewProfile(userProfile) },
  ...
  const talents = [
    {
        ...
        location: "Berlin, Germany", joined: "2023", level: "58",
    ...
    },
    {
        ...
        location: "Toronto, Canada", joined: "2022", level: "45",
        totalPoints: 216000,
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans selection:bg-indigo-500/30 overflow-x-hidden relative">
      ...
      <ProfileModal
        ...
        profile={profileData}
        addToast={addToast}
        actionLabel={profileData?.id !== userProfile.id ? "Initiate Contract" : null}
        onAction={profileData?.id !== userProfile.id ? () => handleSelect(profileData) : null}
      />
      ...
      <main className="pt-32 pb-32 max-w-6xl mx-auto px-6 relative z-10">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {aiSuggestions.candidates.map(talent => (
                                    <SpotlightCard key={talent.id} className="rounded-[32px] p-8 cursor-pointer group" onClick={() => handleViewProfile(talent)}>
                                        <div className="flex justify-between items-start mb-6"><div className="flex items-center gap-4"><div className="w-12 h-12 bg-white/10 rounded-full flex items-center justify-center font-black text-white">{talent.name[0]}</div><div><h4 className="font-bold text-white text-lg">{talent.name}</h4><p className="text-xs text-slate-400 uppercase tracking-wider">{talent.role}</p></div></div><div className="px-3 py-1 bg-indigo-500/20 text-indigo-300 rounded-full text-xs font-black">{talent.matchScore}% Match</div></div>
                                        <p className="text-sm text-slate-300 mb-6 italic">"{talent.reason}"</p>
                                        <div className="flex items-center justify-between border-t border-white/5 pt-4"><p className="text-white font-mono font-bold">{talent.rate.toLocaleString()} PTS <span className="text-slate-500 text-xs">/ day</span></p><button className="bg-white text-black px-6 py-2 rounded-xl font-bold text-sm hover:bg-emerald-400 transition-colors">Hire</button></div>
                                    </SpotlightCard>
                                ))}
                            </div>
                        </div>
        ...
        {view === 'scoping' && selectedItem && (
          <div className="space-y-12 animate-fade-in-up">
            <button onClick={() => setView('marketplace')} className="flex items-center gap-3 text-slate-500 font-black text-xs hover:text-white transition-colors uppercase tracking-[0.2em] group pl-2"><ArrowLeft className="w-3 h-3" /> Abort Sequence</button>
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-4 space-y-6">
                <div className="bg-[#0f172a]/80 rounded-[48px] p-10 border border-white/10 shadow-2xl sticky top-32 backdrop-blur-xl">
                  ...
                  <h2 className="text-5xl font-black italic text-white mb-10 leading-none tracking-tighter">{selectedItem.totalPoints.toLocaleString()} <span className="text-sm not-italic text-slate-500 block mt-2 uppercase tracking-widest font-bold">TrustPoints</span></h2>
                  <button onClick={initiateContract} className="w-full bg-white text-[#020617] py-6 rounded-[24px] font-black text-lg hover:bg-indigo-500 hover:text-white transition-all shadow-xl active:scale-95 flex items-center justify-center gap-2"><Fingerprint className="w-5 h-5 text-slate-400 group-hover:text-white transition-colors" /> Sign & Fund</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* ... (Contract and Wallet views remain the same) ... */}
        {/* VIEW: Contract (Escrow Process) */}
        {view === 'contract' && selectedItem && (
            ...
            <div className="mb-16 flex justify-between items-center max-w-2xl mx-auto relative px-4">
                {steps.map((s, idx) => (
                  <React.Fragment key={s.id}>
                    ...
                    {idx < steps.length - 1 && (<div className="flex-1 h-0.5 bg-slate-800 mx-2 -mt-10 relative overflow-hidden"><div className="h-full bg-indigo-500 transition-all duration-1000" style={{ width: step > s.id ? '100%' : '0%' }} /></div>)}
                  </React.Fragment>
                ))}
            </div>
            ...
            <div className="max-w-4xl mx-auto bg-[#0f172a]/40 rounded-[56px] p-8 sm:p-16 border border-white/[0.05] min-h-[500px] shadow-2xl backdrop-blur-2xl text-center">
                {step === 1 && (
                  <div className="space-y-10 animate-fade-in-up">
                    ...
                    <HoldButton onClick={handleNextStep} label="Activate Trust Stream" icon={ArrowRight} className="w-full max-w-md mx-auto bg-white text-[#020617] py-6 rounded-[32px] font-black text-xl shadow-2xl" color="white" />
                  </div>
                )}
                {step === 2 && (
                  <div className="space-y-10 animate-fade-in-up">
                    ...
                    {isUploading ? (
                        ...
                        <div className="max-w-md mx-auto space-y-4">
                            <div className="border-4 border-dashed border-white/10 rounded-[32px] p-10 flex flex-col items-center justify-center gap-4 hover:border-indigo-500/50 hover:bg-white/5 transition-all cursor-pointer group" onClick={handleFileUpload}>
                                <div className="w-16 h-16 bg-indigo-500/20 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform"><UploadCloud className="w-8 h-8 text-indigo-400" /></div>
                                <h3 className="text-xl font-black text-white">Upload Deliverables</h3>
                            </div>
                             <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">Or skip to next phase</p>
                             <HoldButton onClick={handleNextStep} label="Enter Build Phase" className="w-full bg-indigo-600 text-white py-6 rounded-[32px] font-black text-xl shadow-xl" color="indigo" />
                        </div>
                    )}
                  </div>
                )}
                {step === 3 && (
                   <div className="space-y-10 animate-fade-in-up">
                      <div className="w-32 h-32 bg-amber-500/10 rounded-[48px] flex items-center justify-center border border-amber-500/20 mx-auto animate-pulse"><Scan className="w-14 h-14 text-amber-500" /></div>
                      <h2 className="text-5xl font-black text-white italic tracking-tighter">Neural Inspection</h2>
                      <div className="bg-black/20 p-6 rounded-2xl border border-white/5 max-w-md mx-auto"><p className="text-sm font-bold text-emerald-400 uppercase tracking-widest flex items-center justify-center gap-2"><CheckCircle2 className="w-4 h-4"/> 98.2% Match Verified</p></div>
                      <div className="flex justify-center gap-4 max-w-md mx-auto">
                         ...
                         <HoldButton onClick={handleNextStep} label="Release Funds" className="flex-[2] bg-white text-[#020617] py-6 rounded-[32px] font-black text-xl shadow-xl" />
                      </div>
                   </div>
                )}
                {step === 4 && (
                   <div className="space-y-10 animate-fade-in-up">
                      <div className="w-32 h-32 bg-emerald-500/10 rounded-[48px] flex items-center justify-center border border-emerald-500/20 mx-auto"><Unlock className="w-14 h-14 text-emerald-500" /></div>
                      <h2 className="text-6xl font-black text-white italic tracking-tighter uppercase">Settled</h2>
                      <button onClick={() => { setView('marketplace'); setSelectedItem(null); setStep(1); }} className="px-10 py-4 rounded-full border border-white/10 hover:bg-white/10 text-white font-bold transition-all">Return to Feed</button>
                   </div>
                )}
            </div>
          </div>
        ...
        {view === 'wallet' && (
          <div className="space-y-12 animate-fade-in-up">
            <button onClick={() => setView('marketplace')} className="flex items-center gap-2 text-slate-500 font-black text-xs hover:text-white transition-colors uppercase tracking-[0.2em] pl-2 group"><ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" /> Back to Feed</button>
                  ...
                  <div className="flex flex-col relative z-10 w-full h-full cursor-pointer" onClick={() => setIsFlipped(!isFlipped)}>
                    ...
                    <div className={`transition-all duration-500 ${isFlipped ? 'opacity-100' : 'opacity-0 absolute inset-0'}`}>
                      <div className="flex flex-col h-full justify-between">
                        <div className="flex justify-between items-start"><QrCode className="w-32 h-32 text-white/90" /><RefreshCw className="w-4 h-4 opacity-50" /></div>
                        <div className="space-y-2"><p className="text-[10px] font-black uppercase tracking-widest opacity-60">Wallet Address</p><p className="font-mono text-xs opacity-80 break-all">0x71C...9A21</p></div>
                      </div>
                    </div>
        ...
        {isChatOpen && (
            <div className="w-80 sm:w-96 h-[500px] bg-[#0f172a]/95 backdrop-blur-xl border border-white/10 rounded-[32px] shadow-2xl flex flex-col overflow-hidden animate-fade-in-up">
                <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                    <div className="flex items-center gap-3"><div className="w-10 h-10 bg-indigo-600 rounded-full flex items-center justify-center"><BrainCircuit className="w-5 h-5 text-white" /></div><div><p className="font-black text-white text-sm">Context Chat</p><p className="text-[10px] text-emerald-400 font-bold uppercase tracking-widest">Online</p></div></div>
                        ...
                        <div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                            {msg.type === 'contract_update' ? (
                                <div className="max-w-[90%] w-full bg-indigo-900/40 border border-indigo-500/30 p-5 rounded-[24px] rounded-bl-none">
                                    <div className="flex items-center gap-2 mb-3 text-indigo-300"><FileSignature className="w-4 h-4" /><span className="text-[10px] font-black uppercase tracking-widest">Smart Contract Proposal</span></div>
                                    <h4 className="font-bold text-white mb-2">{msg.data.title}</h4>
                                    <ul className="space-y-1 mb-4">{msg.data.changes.map((change, i) => (<li key={i} className="text-xs text-slate-300 flex items-center gap-2"><div className="w-1 h-1 bg-emerald-400 rounded-full" /> {change}</li>))}</ul>
                                    <div className="flex justify-between items-end mb-4 pt-3 border-t border-white/10"><span className="text-[10px] font-bold text-slate-500 uppercase">Cost Impact</span><span className="text-lg font-black text-white">+{msg.data.additionalCost.toLocaleString()} PTS</span></div>
                                    <div className="flex gap-2"><button className="flex-1 py-2 bg-white/10 rounded-xl text-xs font-bold text-slate-400 hover:text-white">Reject</button><button onClick={() => acceptContractUpdate(msg.data)} className="flex-1 py-2 bg-indigo-600 rounded-xl text-xs font-bold text-white hover:bg-indigo-500">Accept Update</button></div>
                                </div>
                            ) : (
                ...
                <div className="p-4 border-t border-white/5 bg-[#020617]/50">
                    <button onClick={triggerSmartContractUpdate} className="text-[10px] text-slate-500 hover:text-indigo-400 mb-2 w-full text-center uppercase tracking-widest font-bold opacity-50 hover:opacity-100 transition-opacity">[Dev: Simulate Scope Creep]</button>
                    <div className="flex items-center gap-2 bg-white/5 rounded-full px-4 py-2 border border-white/5"><input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()} placeholder="Type a message..." className="bg-transparent border-none outline-none text-white text-sm flex-1 placeholder:text-slate-500" /><button onClick={handleSendMessage} className="p-2 bg-indigo-600 rounded-full text-white hover:bg-indigo-500"><Send className="w-4 h-4" /></button></div>
                </div>
            </div>
        )}
