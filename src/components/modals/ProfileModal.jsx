...
import { User, BadgeCheck, MapPin, Calendar, Hexagon, Activity, ShieldCheck, ArrowRight, Share2, X } from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center pt-12 sm:pt-0 p-2 sm:p-4">
      ...
      <div className="relative w-full max-w-4xl bg-[#0f172a] border border-white/10 rounded-[32px] sm:rounded-[48px] shadow-2xl overflow-hidden animate-scale-up flex flex-col md:flex-row max-h-[90vh]">
        ...
        <button onClick={onClose} className="absolute top-4 right-4 sm:top-6 sm:right-6 p-2 bg-white/5 rounded-full text-slate-400 hover:text-white hover:bg-white/10 transition-colors z-50"><X className="w-5 h-5" /></button>
      </div>
    </div>
  );
};
