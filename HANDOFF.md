...
- **Frontend hosting migrated to Vercel** (session 5, same day as the quality pass) — live at **`https://trustflow.vercel.app`**, git-push-to-deploy from `main`, project `team-kenji/trustflow`. `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` set on both Production and Preview. Verified live, no console errors. Rationale: pure static Vite SPA with zero server-side compute — Cloud Run's Dockerfile/nginx container was pure overhead for this project (unlike a service that actually needs GCP compute).
  - GitHub repo and Vercel project both renamed the same day, dropping the `project-` prefix (naming convention cleanup, unrelated to the hosting migration itself): repo is now `ken2san/trustflow` (local `origin` remote auto-updated by `gh repo rename`; local folder name intentionally left as `project-trustflow`), Vercel project is now `trustflow` → `https://trustflow.vercel.app`. The old `project-trustflow.vercel.app` URL still resolves (Vercel kept it live) but shouldn't be relied on going forward.
